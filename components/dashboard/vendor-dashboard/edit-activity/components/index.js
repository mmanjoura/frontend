...
const Index = ({params}) => {
  ...
  const tabs = [
    ...
    },
    {
      label: "Itinerary",
      labelNo: 4,
      content: <IterneraryTabContent params = {params} category= {"Activity"} />,
    },
    {
      label: "Questions & Answers",
      labelNo: 5,
      content: <QuestionsTabContent params = {params} category= {"Activity"} />,
    },
  ];
      ...
      <TabList className="tabs__controls row x-gap-40 y-gap-10 lg:x-gap-20">
        {tabs.map((tab, index) => (
          <Tab key={index} className="col-auto">
            <button className="tabs__button text-18 lg:text-16 text-light-1 fw-500 pb-5 lg:pb-0 js-tabs-button"  aria-label="Close">
