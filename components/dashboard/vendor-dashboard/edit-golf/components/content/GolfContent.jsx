import DropdownList from '../GolfTypes'; // Adjust the path accordingly
...
const ActivityContent = ({params}) => {
    ...
    useEffect(() => {
      ...
      const fetchActivity = async () => {
        try {
          ...
          const response = await fetch(baseURL + `/golfs/${params.id}`);
          if (!response.ok) {
  ...
  const submit = async (e) => {
    ...
    // tourId = 7;
    const res = await fetch(baseURL + '/golfs/' + params.id, {
      method: "PUT",
      credentials: 'include',
      headers: {
    ...
    <form className="row y-gap-20" onSubmit={submit}>
      ...
      <div className="row x-gap-20 y-gap-20">
        ...
        <div className="form-input" style={{ border: '2px solid #ddd', borderRadius: '4px', width:'400px', height:'50px'}}>
          <DropdownList className="lh-1 text-16 text-light-1" options={golfTypes} onSelect={handleSelect} >
            </DropdownList>
          </div>
        </div>
        <div className="col-12">
        ...
        <div className="col-12">
          <div className="form-input ">
            ...
            <label className="lh-1 text-16 text-light-1">Minimu Duration to finish course</label>
          </div>
        </div>
        ...
        <div className="col-12">
          <div className="form-input ">
            ...
            <label className="lh-1 text-16 text-light-1">Minimu Group Size</label>
          </div>
        </div>
        {/* End Group Size */}
        <div className="col-12">
